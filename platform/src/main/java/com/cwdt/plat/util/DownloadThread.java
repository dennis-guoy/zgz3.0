...
public class DownloadThread extends Thread {
	 ...
	 * 构造方法
	 */
	public DownloadThread(FileDownloader downloader, URL downUrl, File saveFile, int block, int downLength,
			int threadId) {
	...
	public void run() {
		if (downLength < block) {// 未下载完成
			try {
				...
				/****/
				System.out.println("DownloadThread http.getResponseCode()：" + http.getResponseCode());
				if (http.getResponseCode() == 206) {
					/***
					 * //获取输入流 InputStream inStream = http.getInputStream();
					 * byte[] buffer = new byte[1024]; int offset = 0;
					 * print("Thread " + this.threadId + " start download from
					 * position " + startPos);
					 * 
					...
					int offset = 0;
					BufferedRandomAccessFile threadfile = new BufferedRandomAccessFile(this.saveFile, "rwd");
